...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str, default=None, help='checkpoint to load for the model')
    ...
    logging_dir = f"logs-0617/{args.game}_{args.method}_{args.difficulty}/{model}"
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ckpt = args.checkpoint if args.checkpoint is not None else None
        futures = [
            executor.submit(
                main_game_loop, log_file.replace(".log", f"_{j}_{i}.csv") , \
                    ckpt.replace("_0_0.csv", f"_{j}_{i}.csv") if ckpt is not None else None,
                    i, args, next(api_cycle)
            )
