...
def jobs_to_schedule(Args):
    ...
    instance = []
    for s in settings:
        log_dir = f"chess2-logs/{s.replace('-', '_')}"
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        player1_method = s.split('_')[0]
        player2_method = s.split('_')[1]
        slow_model = s.split('_')[2]
        fast_model = s.split('_')[3]
        time_limits = int(s.split('_')[4])
        
        args = argparse.Namespace(
            player1_method=player1_method,
            player2_method=player2_method,
            slow_model=slow_model,
            fast_model=fast_model,
            time_limits=[time_limits, time_limits],
        )
        check_args(args)
