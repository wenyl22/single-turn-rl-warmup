...
if __name__ == "__main__":
    ...
    if args.tagged:
        files = glob.glob("data/Overcooked_tagged/coordination_ring/6_220.csv")
    else:
    ...
    if not os.path.exists(store_path):
        os.makedirs(store_path)
    with open(store_path + "/dataset.jsonl", "w") as f:
        for item in data:
            f.write(json.dumps(item) + "\n")
    print("Dataset generation complete.")
    print("Total number of samples:", len(data))
    print("Dataset saved to:", store_path + "/dataset.jsonl")
