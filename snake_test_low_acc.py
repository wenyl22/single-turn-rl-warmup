...
from envs.snake import state_to_description, llm_state_builder, tick, dir_mapping
from snake_ds_generation import check_valid_move, bfs
import time
...
def process_entry(i, dic, api_key, args):
    from envs.prompts.ma_snake_game import LLM_SYSTEM_PROMPT,  GAME_PROMPT_LOW_LEVEL_2
    ...
    answer = dic['best_actions']
    render = dic['render']
    valid_moves = ast.literal_eval(dic['action_label'])
    # print(valid_moves)
    state_for_llm = llm_state_builder(env.env)
    if args.h == "correct":
        description = state_to_description(state_for_llm, 0, scratch_pad = answer)
    elif args.h == "wrong":
        _ = 1
        while valid_moves[_][0] == True:
            _ += 1
        description = state_to_description(state_for_llm, scratch_pad = dir_mapping[_])
    else:
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages = [
                    ...
                    {"role": "user", "content": GAME_PROMPT_LOW_LEVEL_2 + description},
                ],
                temperature=0.6,
                top_p=0.95,
    ...
    for r in response.choices:
        ...
        action = extract_scratch_pad(text, "", valid_actions="LRDU")[0]
        index = 1 if action == "L" else 2 if action == "R" else 3 if action == "D" else 4 if action == "U" else None
        valid_move = valid_moves[index]
        max_reward = -1
        if valid_move[0]:
            n_state_for_llm, reward  = tick(state_for_llm, action)
            assert n_state_for_llm is not None, f"Invalid action {action} for state {state_for_llm}"
            best_actions, max_reward = bfs(n_state_for_llm, step = 7)
            max_reward += reward
        metrics.append({
            "render": render,
            "snake": dic['snake'],
        ...
        if i < 5:
            with open(args.f.replace(".csv", f"_{i}.txt"), 'a') as f:
                ...
                f.write(f"Safe: {valid_move[0]}, Reward: {max_reward}, Max Reward: {int(dic['max_reward'])}\n")
