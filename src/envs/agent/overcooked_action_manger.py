...
from envs.overcooked_ai_py.mdp.actions import Action, Direction
from envs.agent.overcooked_agent import LLMAgent
import re
...
class LLMActionManager(object):
    ...
    def is_cooking(self, soup):
        # return 0 <= cook_time < 20 and num_items >= self.mdp.num_items_for_soup
        return (not soup.is_ready) and (not soup.is_idle)

    def is_ready(self, soup): 
        return soup.is_ready
