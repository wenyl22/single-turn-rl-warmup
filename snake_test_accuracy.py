        ...
        try:
            response = client.chat.completions.create(
                ...
                messages = [
                    ...
                    {"role": "user", "content": GAME_PROMPT + description},
                ],
                temperature=0.6,
                top_p=0.95,
...
if __name__ == "__main__":
    ...
    if args.f is None:
        ...
        if not os.path.exists(f"logs-0602/{game}-acc/{model_name}"):
            os.makedirs(f"logs-0602/{game}-acc/{model_name}")
        time_stamp = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        args.f = f"logs-0602/{game}-acc/{model_name}/{time_stamp}.csv"
